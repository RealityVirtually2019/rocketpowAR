...
namespace MagicLeap
{
    ...
    public class PlaneVisualizer : MonoBehaviour
    {
        ...
        private void DrawBoundaries(MLWorldPlaneBoundaries[] boundaries)
        {
            ...
            {
                // TODO: BCI Converted from
//                MLWorldPlaneBoundaries? bounds = boundaries.FirstOrDefault(x => x.Id == cache.Id);

                MLWorldPlaneBoundaries? bounds = null;
                foreach (MLWorldPlaneBoundaries mlWorldPlaneBoundary in boundaries)
                {
                    if (mlWorldPlaneBoundary.Id == cache.Id)
                    {
                        bounds = mlWorldPlaneBoundary;
                    }
                }
                if (bounds == null)
                {
